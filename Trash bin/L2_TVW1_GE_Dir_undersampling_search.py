...
retained_ratios = [ 0.10, 0.125, 0.15, 0.20, 0.30, 0.50, 0.75 ]

for retained_ratio in retained_ratios:
    
    print('Current retained_ratio: ', retained_ratio)
    ...
    minima[retained_ratio] = (argmin_key, min_val)
    Pstars_dict[retained_ratio] = Pstars_dict_temp[ argmin_key ]
...
for retained_ratio, (argmin_key, min_val) in minima.items():
...
retained_ratios     = np.array(retained_ratios)
...
sc = ax.scatter(alpha1s, alpha2s, c=min_vals, s=60, edgecolors='k')
cbar = plt.colorbar(sc, ax=ax)
cbar.set_label("min_value")

ax.set_xlabel("alpha1")
ax.set_ylabel("alpha2")
ax.set_title("Best (alpha1, alpha2) per retained_ratio, colored by min_value")

# --- Add retained_ratio labels next to each point ---
for i, (x, y, retained_ratio) in enumerate(zip(alpha1s, alpha2s, retained_ratios)):
    ax.text(x, y+(1+i%3)*1e-3, str(retained_ratio), fontsize=8)  # small offset so text doesn't sit on top
