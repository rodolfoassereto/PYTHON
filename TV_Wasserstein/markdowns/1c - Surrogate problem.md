## Another primal-dual formulation, by dualizing the fidelity

### 0. Old primal-dual problem (no fidelity dualization)



### 1. New primal-dual problem dualizing the fidelity

Using

$\ \tfrac12\|K(P)-E\|^2=\max_h\langle K(P)-E,h\rangle-\tfrac12\|h\|^2
=\max_h\langle K(P),h\rangle-\big(\tfrac12\|h\|^2+\langle E,h\rangle\big)$we get to

$$
\min_{\substack{P,Q}}\max_{f,g,h}\
\underbrace{\mathcal I_{\ge0}(P) + \alpha_1\|Q\|_{2,1}}_{F(P,Q)}
...
**Problem:** the primal-dual gap throughout the iterates of this variational problem may be infinite because some constraint in the primal and dual problems are not enforced by the proximal algorithms. Let us compute the primal and dual forms.
...
where $\bar\delta$ **selects everything *but* the $y$-centers**. Finally:
...
+\underbrace{\mathcal{I}_{\alpha_1 B^{2\infty}}(\nabla_y f)

+\mathcal{I}_{\le0}\big(K^{*}h-\mathbb{J}\operatorname{div}_x f-\mathbb{I}^{*}\operatorname{div} g\big)}_{F^{*}(-\mathcal{K}^{*}(f,g,h))} .
$$
...
The constraints contained in $\mathcal G^* ( \mathcal K(P,Q))$ and $ \mathcal F (-\mathcal K^*(f,g,h)) $ are not enforced in the proximal algorithms and therefore make the primal-dual gap blow to infinity. To compute  a meaningful primal-dual gap, we derive a surrogate problem which is equivalent but has no such insidious constraints.

---

## 2. Surrogate problem (to get a finite primal-dual gap)
...
+\phi_{\alpha_1 C_Q}\big(\alpha_1\|Q\|_{2,1}\big)
...
+\tfrac12\|h\|^2+\langle E,h\rangle\|\big(K^{*}h-\mathbb{J}\operatorname{div}_x f-\mathbb{I}^{*}\operatorname{div} g \big)_+\|_1
+\phi^{*}_{\alpha_1 C_Q}\big(\tfrac 1 \alpha_1\|\nabla_y f\|_{2,\infty}\big).
$
...
#### Dual of the "surrogate" function $\phi_M$
...
\end{cases}
\qquad\text{(at the boundary }M:\ -\tfrac12 M+sM-\tfrac M2=M(s-1)\text{)}, $giving the values
$\begin{cases}
...
\qquad |\Omega|=(N_1+1)(N_2+1), $(both factors of even length), gauge-fix $(x,0)=0\ \ \forall x\in\Omega,\ \forall f:Y\to\mathbb{R}^2 . $ationarity gives $\|\nabla_y f^{*}\|_{2,\infty}\le\alpha_1$, hence $\forall x\in\Omega$,
$\|\nabla f^{*}_x\|_{2,\infty}\le\alpha_1$. Summing increments along a path from the center,
$\|f^{*}\|_\infty \le \alpha_1 \max\{N_1,N_2\}\Big(1+\tfrac{\sqrt2}{2}\Big)=:C_f $
