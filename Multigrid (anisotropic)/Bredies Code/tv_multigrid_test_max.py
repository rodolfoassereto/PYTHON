...
u_true = import_image('abstract_pattern.jpg', 9)
fig = figure(dpi=300)
...
fig = figure(dpi=200)
imshow(u_true[100:300,200:400], cmap='gray', vmin=0, vmax=1)
title('detail original image')
show()

random.seed(42)
f0 = u_true + random.randn(*u_true.shape)*0.2

def aux_plot_3d(mat, minval=None, maxval=None):
    fig = figure()
    ax = fig.add_subplot(111, projection='3d')
    x, y = np.meshgrid(range(mat.shape[0]), range(mat.shape[1]))
    ax.plot_surface(x, y, mat, cmap='rainbow')
    if minval != None and maxval != None:
        ax.set_zlim(minval, maxval)
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z ')
    savefig('myplot.png', dpi=300)
    show()
    return

mat = f0[100:300,200:400].copy()
fig = figure(dpi=200)
ax = fig.add_subplot(111, projection='3d')
x, y = np.meshgrid(range(mat.shape[0]), range(mat.shape[1]))
ax.plot_surface(x, y, mat, cmap='rainbow')
...
alpha0 = 0.2
iter = 20
min_value = 5827.009715479517 # lena
min_value = 5401.008453591635 # abstract_pattern
...
for (i, depth) in enumerate(depths):
    ...
    for i in range(iter):
        ...
        if i % 10 == 0 or i == iter-1:
            ...
            # time.sleep(1e-8)
    imshow(u_[0][100:300,200:400], cmap='gray', vmin=0, vmax=1)
    show()
    fig = figure(dpi=200)
    ax = fig.add_subplot(111, projection='3d')
    toplot3d = u_[0][100:300,200:400]
    x, y = np.meshgrid(range(toplot3d.shape[0]), range(toplot3d.shape[1]))
    ax.plot_surface(x, y, toplot3d, cmap='rainbow')
    ax.set_zlim(0.35,0.7)
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z ')
    title('20 iterations, multigrid')
    show()
    figure(100)
    semilogy((array(values) - min_value)/prod(u_[0].shape))
show()