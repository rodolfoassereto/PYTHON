...
#     print(f"p_norm={pnorm}") # print(f"alpha0={alpha0}, alpha={alpha}")
...
def prepare_data(f0, alpha0, max_depth=0, alpha_fac=1):
    u = []
    v = []
    p = []
    f = []
...
def tv_initial_guess(f, alpha, beta, step):
    ...
    p_ = [None]*length
    for i in range(length):
        u_[i] = u[i] - sigma_step*(v[i] + grad_ad(p[i]))
        v_[i] = v[i] + tau_step*u[i]
        p_[i] = p[i] + tau_step*grad(u[i])
...
def obj_fun(uu, zz, alpha0, fac=1):
    list1 = [(2**i * 0.5 * np.linalg.norm(uu[i]-zz[i]))**2 for i in range(len(uu))]
    list2 = [(2*fac)**i * np.sum(np.abs(grad(uu[i]))) for i in range(len(uu))]
    return np.max(list1) + alpha0 * np.max(list2)

#u_true = imread('lena.png') #[::8,::8]
u_true = bd.import_image('lena.png', 9, stretch=True)
imshow(u_true, cmap='gray', vmin=0, vmax=1)
show()
random.seed(42)
f0 = u_true + random.randn(*u_true.shape)*0.25
imshow(f0, cmap='gray', vmin=0, vmax=1)
show()
alpha0 = 2
dep = 0
alp = 1
(u, v, p, d, f, alpha, beta, step) = prepare_data(f0, alpha0, max_depth=dep, alpha_fac = alp)
...
plots=None
#pdb.set_trace()
obj_val = np.zeros(iter)
for i in range(iter):
    (u, v, p, d, u_) = tv_multigrid_step(u, v, p, d, f, alpha, beta, step)
    if i % 10 == 0 or i == iter-1:
        #u_vis=[2*u_[i] - u[i] for i in range(len(u))]
        imshow(u_[0][:600,:600], cmap='gray', vmin=0, vmax=1)
        title(f"iteration={i}, depth={dep}")
        show()
