...
def _gaussian_density_factory(mu, sig_inv):
    mu = np.asarray(mu)
    return lambda *x: _gaussian_density(mu, sig_inv, *x)

def _Givens_rotation(n, i, j, phi):
    R = np.eye(n); c, s = np.cos(phi), np.sin(phi)
    R[[i,i,j,j],[i,j,i,j]] = c, -s, s, c; return R

def _orthonormal_basis_from_angles(n, phis):
    U = np.eye(n); idx = 0
    for i in range(n-1):
        for j in range(i+1,n):
            U = _Givens_rotation(n, i, j, phis[idx]) @ U; idx += 1
    return U.T

def _generate_field_2d(shape_x, a, b, c=0):
    M, N = shape_x
    X, Y = np.meshgrid(np.linspace(-1,1,M), np.linspace(-1,1,N), indexing='ij')
    return (np.pi/4*(a*X+b*Y+c)).reshape(shape_x+(1,))

def _gen_gauss(eigenvalues, field, shape_y):
    ndim_y = len(shape_y); shape_x = field.shape[:-1]
    P = np.zeros(shape_x+shape_y)
    coords = [np.linspace(-8,8,n) for n in shape_y]
    ...
    for idx in np.ndindex(shape_x):
        ...
        P[idx] = _function_as_array(coords, f)
    return P

def build_gaussian_mixture(shape_x, shape_y):
    return (_gen_gauss((8,0.6), _generate_field_2d(shape_x,0.8,0.8,2), shape_y) +
            _gen_gauss((8,0.6), _generate_field_2d(shape_x,0,0,-0.75), shape_y))

def undersampling_mask_xy(shape_x, shape_y, retained_ratio, concentration_coeff):
    ndim_y = len(shape_y); size_y = np.prod(shape_y)
    N_ones = int(retained_ratio * size_y)
    f = _gaussian_density_factory(ndim_y*[0], np.eye(ndim_y))
    dom = 4*concentration_coeff*np.pi/2
    coords = [np.linspace(-dom,dom,n) for n in shape_y]
    probs = _function_as_array(coords, f).flatten(); probs /= probs.sum()
    ...
    mask_xy = np.zeros(shape_x+shape_y, dtype=bool); mask_xy[:] = mask_y
    return mask_xy
